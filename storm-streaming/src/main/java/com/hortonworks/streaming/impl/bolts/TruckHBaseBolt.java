...
public class TruckHBaseBolt implements IRichBolt {
	...
	/**
	 * We don't need to set any configuration because at deployment time, it
	 * should pick up all configuration from hbase-site.xml as long as it in
	 * classpath. Note that we store hbase-site.xml in src/main/resources so it
	 * will be in the topology jar that gets deployed
	 *
	 * @return
	 */
	...
	public void execute(Tuple input) {
		...
		if (!eventType.equals("Normal")) {
			try {
				...
				incidentTotalCount = this.eventsCountTable.incrementColumnValue(Bytes.toBytes(driverId),
						Bytes.toBytes(EVENTS_COUNT_TABLE_COLUMN_FAMILY_NAME), INCIDENT_RUNNING_TOTAL_COLUMN, 1L);
	...
	private Put constructRow(String columnFamily, int driverId, int truckId, Timestamp eventTime, String eventType,
		...
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(driverColumn), Bytes.toBytes(driverId));
		String truckColumn = "truckId";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(truckColumn), Bytes.toBytes(truckId));

		String eventTimeColumn = "eventTime";
		long eventTimeValue = eventTime.getTime();
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(eventTimeColumn), Bytes.toBytes(eventTimeValue));

		String eventTypeColumn = "eventType";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(eventTypeColumn), Bytes.toBytes(eventType));

		String latColumn = "latitudeColumn";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(latColumn), Bytes.toBytes(latitude));

		String longColumn = "longitudeColumn";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(longColumn), Bytes.toBytes(longitude));

		String driverNameColumn = "driverName";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(driverNameColumn), Bytes.toBytes(driverName));

		String routeIdColumn = "routeId";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(routeIdColumn), Bytes.toBytes(routeId));

		String routeNameColumn = "routeName";
		put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(routeNameColumn), Bytes.toBytes(routeName));
		return put;
	}
