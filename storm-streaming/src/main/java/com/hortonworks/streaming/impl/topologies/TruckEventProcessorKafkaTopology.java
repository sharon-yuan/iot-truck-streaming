...
public class TruckEventProcessorKafkaTopology extends BaseTruckEventTopology {
	...
	public void WSYBuildAndSubmit() {
		TopologyBuilder builder = new TopologyBuilder();
		LOG.info("start config");
		configureKafkaSpout(builder);
		if (topologyConfig.getProperty("kafka.topic").contains("msg"))
			msgFlowBuildAndSubmit(builder);
		else
			logFlowBuildAndSubmit(builder);
	}

	@SuppressWarnings("deprecation")
	public void msgFlowBuildAndSubmit(TopologyBuilder builder) {

		/*
		 * Setup HBse Bolt for to persist violations and all events (if
		 * configured to do so)
		 */
	...
	@SuppressWarnings("deprecation")
	public void logFlowBuildAndSubmit(TopologyBuilder builder) {
	...
	/**
	 * Send truckEvents from same driver to the same bolt instances to maintain
	 * accuracy of eventCount per truck/driver
	 *
	 * @param builder
	 */
	public void configureMonitoringBolt(TopologyBuilder builder) {
		int boltCount = Integer.valueOf(topologyConfig.getProperty("bolt.thread.count"));
		builder.setBolt("monitoring_bolt", new TruckEventRuleBolt(topologyConfig), boltCount)
	...
	public int configurePredictionBolt(TopologyBuilder builder) {
		...
		if (topologyConfig.getProperty("prediction.engine").equalsIgnoreCase("jpmml")) {
			...
			builder.setBolt(predictionBoltName, new PredictionBoltJPMML(topologyConfig), 1)
 ...
 public void configureWebSocketBolt(TopologyBuilder builder) {
		boolean configureWebSocketBolt = Boolean.valueOf(topologyConfig.getProperty("notification.topic"))
				.booleanValue();
